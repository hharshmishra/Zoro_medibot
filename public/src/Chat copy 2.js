...
const [responseChatLog, setResponseChatLog] = useState([]);
  ...
  async function HandleSubmit (e) {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setResponseChatLog([...responseChatLog, {zoro : true , message: `${response}`}])

        // Start printing response letter by letter
        let charIndex = 0;
        setCurrentChar(charIndex);

        const printResponseInterval = setInterval(() => {
          charIndex += 1;
          setCurrentChar(charIndex);
          console.log(charIndex);
          if (charIndex >= response.length) {
            clearInterval(printResponseInterval);
            setIsZoroResponding(false);
          }
        }, 100); // Adjust the interval for typing speed
      }, 2000); // Simulated response time from Zoro
    }, 1000); // Simulated "Zoro is typing" time
    setTypingMessage("Hey! I'm Zoro, Your Personalised Medical Chatbot. How can I help?");
  };
  useEffect(() => {
    // While printing, update the response message with characters up to the currentChar index
    if (isZoroResponding && responseMessage) {
      setResponseMessage(responseMessage.slice(0, currentChar));
    }
  }, [currentChar, isZoroResponding, responseMessage]);
      ...
      <section className = 'chatbox'>
        <div>
          <div className='chat-section'>
            ...
            {responseChatLog.map((message, index) => (
              <ResponseChatMessage key={index} className={`${message.zoro}`} message = {message}/>
            ))}
            {/* </div> */}
...
const ResponseChatMessage = ({message}) =>{
  return(
    <div className = "chat-message-center">
